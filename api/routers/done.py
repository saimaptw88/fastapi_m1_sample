...
import api.schemas.done as doen_schema


router = APIRouter()


@router.put("/tasks/{task_id}/done", response_model=doen_schema.DoneResponse)
async def mark_task_as_done(task_id: int, db: AsyncSession=Depends(get_db)):
  done = await done_crud.get_done(db, task_id=task_id)

  if done is not None:
    raise HTTPException(status_code=400, detail="Done already exist")
